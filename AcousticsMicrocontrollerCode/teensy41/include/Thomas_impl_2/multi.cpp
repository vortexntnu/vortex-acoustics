...
Pos multi(std::vector<Pos> hydrophone_array, std::vector<double> TDOA, const double speed_of_sound) {
    ...
    if (TDOA.size() < 4 || hydrophone_array.size() < 4) {
        std::cout << "Error: Not enough hydrophones or TDOA values for multilateration. You had " << TDOA.size() << " TDOA values and " << hydrophone_array.size() << " hydrophones.\n";
...
Pos::Pos(std::vector<double> pos): x(pos.at(0)), y(pos.at(1)), z(pos.at(2)){
    if (pos.size() >= 4){
        dist = pos[3];
        if (pos.size() > 4){
            std::cout << pos.size()-4 << " values where not used in converting vector to Pos" << std::endl;
        }
    }
