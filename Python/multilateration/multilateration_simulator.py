...
def calculate_tdoa(ref, pos):
    return (1 / v) * (
        math.sqrt(
            (pos[0] - pinger_pos[0]) ** 2
            + (pos[1] - pinger_pos[1]) ** 2
            + (pos[2] - pinger_pos[2]) ** 2
        )
        - math.sqrt(
            (ref[0] - pinger_pos[0]) ** 2
            + (ref[1] - pinger_pos[1]) ** 2
            + (ref[2] - pinger_pos[2]) ** 2
        )
    )
...
def compare_answer(calculated_coordinates):
    ...
    print(
        ...
        [
            ...
            pinger_pos[2] - calculated_coordinates[2],
        ],
    )
