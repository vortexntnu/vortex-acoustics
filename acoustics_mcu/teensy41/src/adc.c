...
const uint32_t ADC_reg_config = (1 << CONFIG_WRITE_EN) | (1 << CONFIG_PD_D) |
...
int16_t *samples_raw_hydrophones[5] = {samples_raw_hydro_1, samples_raw_hydro_2,
                                       samples_raw_hydro_3, samples_raw_hydro_4,
                                       samples_raw_hydro_5};

volatile uint8_t stop_sampling;

volatile uint8_t active_buffer =
    0; // to know which one is being filled, [0, BUFFER_PER_CHANNEL-1]
...
static inline void busyint_interrupt_init(void) {
  GPIO7->ISR |= (1 << BUSYINT);
  GPIO7->ICR2 |= GPIO_ICR2_ICR18(0x3); // falling edge
  GPIO7->EDGE_SEL &= (1 << BUSYINT);
  GPIO7->IMR |= (1 << BUSYINT);

  NVIC_SetPriority(GPIO6_7_8_9_IRQn, 2);
}

static void adc_config(uint32_t reg_val) {
  // pins as output
  // configPort(DB_GPIO_PORT_NORMAL-> 0xFFFF0000, DB_MASK);
  DB_GPIO_PORT_NORMAL->GDIR =
      (DB_GPIO_PORT_NORMAL->GDIR & ~DB_MASK) | (DB_MASK);
...
void read_loop() {
  ...
  for (uint16_t hydrophone = 0; hydrophone < N_HYDROPHONES; hydrophone++) {
    ...
    samples_raw_hydrophones[hydrophone][index] =
        DB_GPIO_PORT_NORMAL->PSR > DB_REG_SHIFT;
    GPIO9->DR_SET |= (1 << _RD);
...
/**
   @brief is starting the major loop timer, PIT0 that will trigger the
   conversion until stopped
   @param sample_period_us : a conversion will happen every "sample_period_us"
   microseconds
...
void adc_start_conversion(float sample_period_us, ADC_sample_mode sample_mode) {
  ...
  // ? do it in one call?
  // setUpPeriodicISR(triggerConversion,
  // clock_ADC::get_clockcycles_micro(1000000), PIT_0); value found by trial and
  // error.
  ...
  setUpPeriodicISR(adc_trigger_conversion,
                   get_clockcycles_micro(bounded_period), PIT_0);
...
void adc_stop_conversion() {
  ...
  // detachInterrupt(BUSYINT_ARDUINO_PIN);
  NVIC_DisableIRQ(GPIO6_7_8_9_IRQn);
...
void adc_trigger_conversion() {
  ...
  // conversion on all channels stopwatch = elapsedMicros(); clk_cyc =
  // ARM_DWT_CYCCNT;
