...
#define SAMPLE_FREQUENCY                                                       \
  112500.0f /* Sample frequency                     [Hz]      */
#define SAMPLE_TIME                                                            \
  1 / SAMPLE_FREQUENCY /* Sample time                          [s]       */

#define IN_BUFFER_LENGTH                                                       \
  4096u /* Total number (real + imaginary) measurements   */
        /* per hydrophone                                 */
#define DMA_BUFFER_LENGTH                                                      \
  IN_BUFFER_LENGTH / 2 /* Number real measurements transferred with DMA  */
#define IIR_SIZE                                                               \
  IN_BUFFER_LENGTH /* Number of data-points to filter                */
...
#define STARBOARD_HYD_X                                                        \
  0.11f /* x - position of starboard hydrophone [m]       */
#define STARBOARD_HYD_Y                                                        \
  0.31f /* y - position of starboard hydrophone [m]       */
#define STARBOARD_HYD_Z                                                        \
  0.15f /* z - position of starboard hydrophone [m]       */
...
#define MARGIN_POS_ESTIMATE                                                    \
  0.5f /* Error tolerable in estimating the position [m] */
#define MARGIN_POS_SEARCH                                                      \
  0.25f /* How much the search will deviate in x      [m] */

#define MARGIN_INTENSITY                                                       \
  20 /* Difference between the intensity-measurements                          \
      */
#define MARGIN_TIME_EPSILON                                                    \
  0.1f /* Determines the difference allowed between the  */
...
#define CURR_TESTING_BOOL                                                      \
  0u /* Bool to indicate if the code is being tested   */
     /*    Not testing: 0u                             */
     /*    Testing    : 1u                             */
 ...
 *
 * NOTE: Since the function uses a biquad-filter, every filter with order > 2 is
 * split into multiple serial second-order filters, leaving at most one filter
 * with order = 1
 *
 * Example:
 ...
 *
 * NOTE: The MATLAB-script used to test the filter can be found in the Matlab
 * folder in in the main directory.
 *
 * @param num_stages            Number of second order cascade-filters.
 * Determines the filter order. Order = 2 * num_stages
 *
 * @param state_coefficients    Initial values for x[n-1], x[n-2], ...,
 * x[n-(2*num_stages)] y[n-1], y[n-2], ..., y[n-(2*num_stages)]
 *
 * @param filter_coefficients   Filter coefficients given as {b10, b11, b12,
 * a11, a12 b20, b21, b22, a21, a22, ...}
 *
