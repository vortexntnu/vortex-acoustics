...
/*
    *Pin definition of Control signals
    The CORE_PINX_BIT define is the offset in the GPIO register. For example
   CORE_PIN39_BIT and CORE_PIN34_BIT are 29, but they are on a different GPIO
   port (1 and 2, respectively) To access the right control and output registers
   we need the corresponding port. GPIO1 and GPIO6 are the same port, GPIO6
   operates the pins in fast mode //!(not compatible with DMA) Same with GPIO2
   and GPIO7 and so on. *see chapter about IO, chapter 10, 11 and especially 12
   from datasheet
*/
...
#define BUSYINT_ARDUINO_PIN                                                    \
    36 // pin number to use for built-in arduino libraries
...
// The pins are rearranged in a way to have the values from adc in the GPIO 1
// (or 6 in fast mode) the data will be at bits [16:31], so we only need a shift
// of 16.
...
static uint16_t sampleData[N_CHANNELS]; // array where the measurements will be
                                        // stored before put into ringbuffer
...
void startConversion(
    ...
    ADC_sample_mode sample_mode = BLOCKING); // setup periodic timer interrupts.
