...
/// this was to slow down the systemclock to be able to analyse the data with
/// the oscilloscope at the office might need to be checked again nice graphic p
/// 1016
...
void setup() {
    ...
    CCM_CBCMR |=
        CCM_CBCMR_PRE_PERIPH_CLK_SEL(3); // Using PPL2 as source clock (~600MHz
                                         // in theory) (~528MHz in practice)
    // derive Peripheral from Pre-Peripheral Clock
    CCM_CBCDR &=
        ~CCM_CBCDR_PERIPH_CLK_SEL;      // Select clock from PRE_PERIPH_CLK_SEL
    CCM_CBCDR |= CCM_CBCDR_AHB_PODF(0); // divide /1 -> AHB Clock
    CCM_CBCDR |= CCM_CBCDR_IPG_PODF(
        3); // divide /4 (Unable to change -Christian) (~150Mhz at this point)
...
// Clock: 150 MHz -> Period: 6.667 ns (IN THEORY)
    ...
    CCM_CBCDR |=
        CCM_CBCDR_AHB_PODF(2); // divide /3 -> AHB Clock (~200Mhz at this point)
    CCM_CBCDR |= CCM_CBCDR_IPG_PODF(
        3); // divide /4 (Unable to change -Christian) (~50Mhz at this point)
    CCM_CSCMR1 &= ~CCM_CSCMR1_PERCLK_CLK_SEL;
    CCM_CSCMR1 |= ~CCM_CSCMR1_PERCLK_PODF(0); // divide /1
