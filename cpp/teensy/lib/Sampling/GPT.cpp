...
void setup() {
    ...
    // Enables GPT1 module and resetting of counter register after resetting EN
    // bit (0th-bit)

    GPT1_IR |= GPT_IR_OF1IE;

    attachInterruptVector(IRQ_GPT1,
                          ISR); /// ISR() is the interupt routine specific to
                                /// this namespace (--> gpd::ISR())
    NVIC_ENABLE_IRQ(
        IRQ_GPT1); /// activating interupt management for this interrupt
}
...
void setUpGptISR(void_function_ptr function) {
    isr_timer_func =
        function; /// sets function that will be called when timer finished
}
}; // namespace gpt
