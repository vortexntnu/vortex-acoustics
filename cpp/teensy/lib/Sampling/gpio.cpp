...
/*
    Basic explanations:
    By including core_pins.h in the header file, we have access to register
   names of Teensy

*/

/** @brief set pin to input or output mode. 1->output. 0->input.
 * @param pin: should be of type CORE_PINX_BIT, i.e. the pins number on the
 * teensy.
...
void configPort(IMXRT_GPIO_t& GPIO_n, uint32_t reg_val,
...
/**
 * @brief By default, Arduino sets pin to fast mode. Pins are configured to
 * normal speed. Only GPIO ports 1-5 can then be used See page 375 for register
 * description
...
void set_normal_GPIO(uint32_t mask, IMXRT_GPIO_t& GPIO_n) {
    ...
    } else if (&GPIO_n == IMXRT_GPIO2_ADDRESS ||
               &GPIO_n == IMXRT_GPIO7_ADDRESS) {
        IOMUXC_GPR_GPR27 &= ~mask; // 0xFFFFFFFF to use fast GPIO
    } else if (&GPIO_n == IMXRT_GPIO3_ADDRESS ||
               &GPIO_n == IMXRT_GPIO8_ADDRESS) {
        IOMUXC_GPR_GPR28 &= ~mask; // 0xFFFFFFFF to use fast GPIO
    } else if (&GPIO_n == IMXRT_GPIO4_ADDRESS ||
               &GPIO_n == IMXRT_GPIO9_ADDRESS) {
...
void write_pin(int pin, uint8_t value, IMXRT_GPIO_t& GPIO_n) {
    if (value) {
        GPIO_n.DR_SET |=
            (1 << pin); // the MCU has registers to set or clear registers
    } else {
...
void write_port(uint32_t reg_value, IMXRT_GPIO_t& GPIO_n, uint32_t mask) {
    ...
    GPIO_n.DR_CLEAR =
        ~reg_value &
        mask; // flipping bits to set the bits that needs to be cleared
}

uint32_t read_port(IMXRT_GPIO_t& GPIO_n) { return GPIO_n.PSR; }
