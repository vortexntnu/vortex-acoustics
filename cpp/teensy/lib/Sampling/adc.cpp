...
*---- from ADC datasheet -----
*    init():
        -configuring the pins: maybe need to reconfigure from write to read -->
test -CS(13), RD(12), WR(11), PAR/SER(8), HW/SW(37) CONVST as output,
BUSY/INT(35) as input -floating pins in software: XCLK/RANGE, REFEN -settings
pins to enable software and parallel mode Config ADC: -finding the right
register value -write value to register
*    config_adc():
        ...
        IF bit 30, the register values can be read (maybe on parallel interface)
on SDO_A -PAR-pins as input again
*    start_convertion():
        ...
        -if using BUSY: max time to BUSY high: 25ns. Then stays high for max 1,7
micro sec depending on ADC model (12-14-16 bit), don't care about time, using
BUSY pin
*    beginRead():
...
// should not be accessed from outside, get called when triggerconversion is
// called. trigger conversion can be called from the outside to sample only
// once.
void beginRead();
void readData();
...
// set up sampling
void setup() {
    clock::setup(); /// the clockfrequency needs to be defined somewhere, does
                    /// it need to be called also if adc is not init()
    PIT::setup();
    ...
    PIT::setUpPeriodicISR(readData, clock::get_clockcycles_nano(T_RDL * 20),
                          PIT::PIT_1);
    Serial.print("Clockcycles PIT1 : ");
    Serial.println(clock::get_clockcycles_nano(T_RDL * 20));
    PIT::setUpPeriodicISR(next_RD, clock::get_clockcycles_nano(T_RDH * 20),
                          PIT::PIT_2);
    Serial.print("Clockcycles PIT2 : ");
    Serial.println(clock::get_clockcycles_nano(T_RDH * 20));
    // ! connect beginRead() to BUSY/INT interrupt -> is done in
    // trigger_conversion()
}
...
/// @brief function to start the conversion of data from ADC. once ADC is ready
/// to output data, GpioISR will be triggered by the BUSY pin
void triggerConversion() {
    ...
    // will pull the CONVST line high, that indicates to the adc to start
    // conversion on all channels
    ...
    switch (ADC_mode) {
    case BLOCKING:
        attachInterrupt(digitalPinToInterrupt(BUSYINT_ARDUINO_PIN), read_loop,
                        FALLING);
        break;
    case TIMER:
        attachInterrupt(digitalPinToInterrupt(BUSYINT_ARDUINO_PIN), beginRead,
                        FALLING);
        break;
    default:
        break;
...
void beginRead() {
    // Serial.println("b");
    //  ! disable interrupts from BUSY/INT, will be enabled again by next
    //  triggerConvst()
...
#ifndef TESTING
    // sampleData[channels_processed] = read_ADC_par(); /// sampledata len = 8,
    // maybe change if not needed (readloop would only need len = 5)
...
void setting_up_timers_DMA() {
    ...
    // OEN: OFLAG is driven on external pin, FORCE: val is forced to OFLAG
    // output, MAster mode to reset other channels when this one is resetting
    ...
    // CM (count mode): counting rising edges of primary source, PCS (Primary
    // count source): IP bus clock with prescaler 1, LENGHT=1: counts until
    // compare, OUTMODE: Toggle output on compare ! for testing prescaler (it
    // was 8 before)
    TMR4_CTRL0 = TMR_CTRL_CM(1) | TMR_CTRL_PCS(15) | TMR_CTRL_LENGTH |
                 TMR_CTRL_OUTMODE(3);
    ...
    // CM (count mode): counting rising edges of primary source, PCS (Primary
    // count source): IP bus clock with prescaler 1, LENGHT=0: counts until
    // rollover, OUTMODE: Toggle output on compare COINIT: reinitialization when
    // channel 0 is resetting
    TMR4_CTRL1 = TMR_CTRL_CM(1) | TMR_CTRL_PCS(15) | TMR_CTRL_COINIT |
                 TMR_CTRL_OUTMODE(3);
    ...
    TMR4_CTRL2 = TMR_CTRL_CM(1) | TMR_CTRL_PCS(8) | TMR_CTRL_COINIT |
                 TMR_CTRL_OUTMODE(3);
...
void setting_up_DMA_channels() {
    ...
    // dma1.TCD->SOFF = 8;
    // dma1.TCD->ATTR = DMA_TCD_ATTR_SSIZE(3) | DMA_TCD_ATTR_SMOD(4) |
    // DMA_TCD_ATTR_DSIZE(2); dma1.TCD->NBYTES_MLOFFYES = DMA_TCD_NBYTES_DMLOE |
    ...
    // dma3.TCD->SOFF = 8;
    // dma3.TCD->ATTR = DMA_TCD_ATTR_SSIZE(3) | DMA_TCD_ATTR_SMOD(4) |
    // DMA_TCD_ATTR_DSIZE(2); dma3.TCD->NBYTES_MLOFFYES = DMA_TCD_NBYTES_DMLOE |
    //                             DMA_TCD_NBYTES_MLOFFYES_MLOFF(-65536) |
    //                             DMA_TCD_NBYTES_MLOFFYES_NBYTES(16);
    // dma3.TCD->SLAST = 0;
