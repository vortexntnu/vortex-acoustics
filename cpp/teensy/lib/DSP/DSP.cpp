...
q31_t **peakDetection(q15_t *results, int16_t countTest) {
  ...
  /*We make an array with the results sorted for use in the median calculation.
  A big part of the 510k Hz zone is either 0 or very low, so not only is is
  performance wise wasteful, it is also for calculating a proper median*/
  ...
  does not work seem well with the sizeof() function for arrays. This is why the
  array has 1 extra index. */
  numNonNull++;
